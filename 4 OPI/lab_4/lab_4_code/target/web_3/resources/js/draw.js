...
canvasPlot.addEventListener("click", function (event) {
    if (currentR === undefined) {
        document.getElementById("error_message").textContent = "Выберите значение R!";
        return;
    }
    ...
    if (tableX < -4 || tableX > 2) {
        document.getElementById("error_message").textContent = "Значение X не соответствует указанному диапазону!";
    } else if (tableY < -3 || tableY > 3) {
        document.getElementById("error_message").textContent = "Значение Y не соответствует указанному диапазону!";
    } else {
        document.getElementById("error_message").textContent = "";
        sendData(tableX, tableY);
    }
